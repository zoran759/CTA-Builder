    ...
    return (
      <div className={`cta-design preview ${isActive ? 'active' : ''}`}>
        ...
        <div className={`cta-screen-imitation ${!this.ifOnlyImage() ? isDesktop ? "desktop" : "phone" : ''}`}>
          <div className={`cta-content-container ${!isDesktop && !this.ifOnlyImage() ? "mobile" : ''} ${!this.ifOnlyImage() ? behavior.position : ''}`} style={!this.ifOnlyImage() ? { bottom: Number(behavior.bottom)+70 + "px", left: Number(behavior.left)+16 + "px", right: behavior.right+16 + "px" } : {}}>
            <div className={`cta-content ${this.ifTriggerButton() ? "d-none" : ''} ${!data.logo && data.image ? "without-logo" : ''}`} style={{ width: data.width + "px", background: data.background, border: data.stroke ? data.stroke : "none", borderRadius: data.corner + "px", borderColor: data.stroke, boxShadow: data.shadow }}>
            ...
            <div className={`cta-trigger-button-container ${behavior.position}`}>
              {
                !this.ifOnlyImage() ?
                  (
                    <div className={`cta-content-button trigger ${data.triggerButtonType} ${data.triggerButtonAlign}`} style={{
